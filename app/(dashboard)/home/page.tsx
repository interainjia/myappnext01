...
export default function HomePage() {
  ...
  const stats = [
    { label: 'Active Projects', value: '12', icon: Briefcase, color: 'text-blue-600 dark:text-blue-400', bg: 'bg-blue-50 dark:bg-blue-950/40' },
    { label: 'Pending Reviews', value: '5', icon: Activity, color: 'text-amber-600 dark:text-amber-400', bg: 'bg-amber-50 dark:bg-amber-950/40' },
    { label: 'Completion Rate', value: '84%', icon: TrendingUp, color: 'text-emerald-600 dark:text-emerald-400', bg: 'bg-emerald-50 dark:bg-emerald-950/40' },
    { label: 'Next Milestone', value: 'Oct 24', icon: Calendar, color: 'text-purple-600 dark:text-purple-400', bg: 'bg-purple-50 dark:bg-purple-950/40' },
  ];
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500">
      {/* Welcome Section */}
      <section className="bg-white dark:bg-slate-900 rounded-2xl p-8 shadow-sm border border-slate-100 dark:border-slate-700 flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div>
      ...
      {/* Quick Links Section */}
      <section>
        <h2 className="text-xl font-bold text-slate-900 dark:text-slate-100 mb-6">Quick Navigation</h2>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {quickLinks.map((link, i) => (
            <Link 
              key={i} 
              href={link.href}
              className="group bg-white dark:bg-slate-900 p-6 rounded-2xl shadow-sm border border-slate-100 dark:border-slate-700 hover:border-[#4db694] hover:shadow-md transition-all flex items-start gap-4"
            >
              <div className="w-12 h-12 bg-slate-50 dark:bg-slate-800 text-slate-400 dark:text-slate-500 group-hover:bg-[#4db694]/10 group-hover:text-[#4db694] rounded-xl flex items-center justify-center shrink-0 transition-colors">
                <link.icon size={24} />
              </div>
              <div>
                <h3 className="text-lg font-semibold text-slate-900 dark:text-slate-100 group-hover:text-[#4db694] transition-colors">
                  {link.name}
                </h3>
                <p className="text-slate-500 dark:text-slate-400 mt-1">{link.description}</p>
              </div>
